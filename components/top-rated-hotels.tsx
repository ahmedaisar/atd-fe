...
function Stars({ value, className = '' }: { value?: number; className?: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-0.5 ${className}`} aria-label={`${rounded} star${rounded === 1 ? '' : 's'}`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <svg
          ...
          viewBox="0 0 20 20"
          fill="currentColor"
          className={`w-3.5 h-3.5 ${i < rounded ? 'text-yellow-400' : 'text-gray-300'}`}
          aria-hidden="true"
        >
...
export interface TopRatedHotel {
  ...
  ctaHref?: string
  // Optional quality info (e.g., star rating)
  quality?: {
    stars?: number
  }
}
