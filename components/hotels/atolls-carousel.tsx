...
export function AtollsCarousel() {
  const [currentSlide, setCurrentSlide] = useState(0)
    ...
    // Only initialize keen-slider when atolls are loaded and non-empty
    const keenConfig = atolls.length > 0 ? {
      initial: 0,
    ...
    // Navigation helpers must be in scope for button handlers
    const prevSlide = () => {
    ...
    // Only render slider UI if atolls are loaded and non-empty
    if (atolls.length === 0) {
      return null;
    }
    return (
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative">
            {/* Carousel Container */}
            <div className="overflow-visible mx-0 sm:mx-6 lg:mx-12 relative z-0">
              {atolls.length > 0 && (
                    ...
                    <div key={idx} className="keen-slider__slide px-2 sm:px-3 lg:px-4">
                      <Card className="overflow-hidden hover:shadow-xl transition-all duration-300 group cursor-pointer border-0 bg-white/80 backdrop-blur-sm h-[480px] flex flex-col">
                        ...
                        <CardContent className="p-6 flex flex-col flex-1">
                          ...
                          <div className="flex flex-wrap gap-2 mb-4 overflow-visible whitespace-nowrap">
                          ...
                          <div className="mt-auto">
                            <Button className="w-full bg-blue-500 hover:bg-blue-700 text-white">Explore {atoll.name}</Button>
                          </div>
                        </CardContent>
                      </Card>
                    </div>
            ...
            {/* Navigation Buttons */}
            {/* Navigation Buttons: always render if atolls.length > perView */}
            {(() => {
              const details = instanceRef.current?.track?.details;
              const options = instanceRef.current?.options;
              const perView = options && options.slides && typeof options.slides === "object" && 'perView' in options.slides ? (options.slides as { perView?: number }).perView ?? 3 : 3;
              if (!details || !options || atolls.length <= perView) return null;
              return <>
                <Button
                  variant="outline"
                  size="icon"
                  className="absolute left-0 top-1/2 -translate-y-1/2 z-30 bg-white shadow-lg hover:bg-gray-50"
                  onClick={(e) => {
                    e.stopPropagation();
                    prevSlide();
                  }}
                ...
                <Button
                  variant="outline"
                  size="icon"
                  className="absolute right-0 top-1/2 -translate-y-1/2 z-30 bg-white shadow-lg hover:bg-gray-50"
                  onClick={(e) => {
                    e.stopPropagation();
                    nextSlide();
                  }}
                  disabled={currentSlide === details.slides.length - perView}
                  aria-label="Next"
                >
            ...
            {/* Dots Indicator */}
            {loaded && instanceRef.current && instanceRef.current.track && instanceRef.current.track.details && (
              <div className="flex justify-center mt-6 gap-2">
                {(() => {
                  const details = instanceRef.current?.track?.details;
                  const options = instanceRef.current?.options;
                  if (!details || !options) return null;
                  const perView = options.slides && typeof options.slides === "object" && 'perView' in options.slides ? (options.slides as { perView?: number }).perView ?? 3 : 3;
                  const dots = details.slides.length - perView + 1;
                  return Array.from({ length: dots }).map((_, idx) => (
                    <button
                      key={idx}
